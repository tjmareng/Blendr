...
def update_profile(request):
    if request.method == 'POST':
        ...
        sexuality = request.POST.get('sexuality')
        current_user = user_info(request.COOKIES.get('user_id_token'))
        print(username + " " + biography + " " + gender + " " + sexuality)
        current_user_cleaned_email = clean_email(current_user['email'])
        db.child("users").child(current_user_cleaned_email).child("username").set(username)
        db.child("users").child(current_user_cleaned_email).child("biography").set(biography)
        db.child("users").child(current_user_cleaned_email).child("gender").set(gender)
        db.child("users").child(current_user_cleaned_email).child("sexuality").set(sexuality)
